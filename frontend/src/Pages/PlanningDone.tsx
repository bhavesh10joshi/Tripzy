...
export function DonePlanning() {
    ...
    useEffect(function () {
        if (!ScriptLoaded || PlanData.length === 0 || !MapRef.current) return;

        const firstTrip = PlanData[0];
        if (!firstTrip.events || firstTrip.events.length === 0) return;
        ...
        const firstDayEvents = firstTrip.events[0]?.Events || [];
        
        if (firstDayEvents.length < 2) {
            geocoder.geocode({ address: firstTrip.PlaceName }, (results, status) => {
        ...
        const addressList = firstDayEvents.map((e: any) => `${e.NameOfEvent}, ${firstTrip.PlaceName}`);
        
        const geocodePromises = addressList.map((address: string) => {
        ...
        Promise.all(geocodePromises.map((p: any) => p.catch(() => null)))
            .then((locations) => {
            ...
            <>
                {PlanData.map((trips: any, index: number) => (
                    <div key={index} className="px-4 md:px-[2rem] pt-4 md:pt-[2rem] animate-in fade-in duration-700">
                        <div className="flex flex-col md:flex-row justify-center items-center md:items-stretch relative">
                            ...
                            <div className="p-4 md:p-[2rem] w-full md:w-1/2 flex flex-col md:flex-row justify-center md:justify-end items-center gap-6 mt-4 md:mt-[-3rem]">
                                ...
                                <div className="flex flex-col gap-3 w-full max-w-[14rem] md:self-start md:mt-[3rem]">
                                    <button className="flex items-center justify-center gap-2 bg-[#0052cc] hover:bg-[#0043a4] text-white font-bold text-sm py-3 px-5 rounded-full shadow-md transition-colors w-full">
                                        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                                            <path d="M9 21c0 .55.45 1 1 1h4c.55 0 1-.45 1-1v-1H9v1zm3-19C8.14 2 5 5.14 5 9c0 2.38 1.19 4.47 3 5.74V17c0 .55.45 1 1 1h6c.55 0 1-.45 1-1v-2.26c1.81-1.27 3-3.36 3-5.74 0-3.86-3.14-7-7-7zm2.85 11.1l-.85.6V16h-4v-2.3l-.85-.6C7.8 12.16 7 10.63 7 9c0-2.76 2.24-5 5-5s5 2.24 5 5c0 1.63-.8 3.16-2.15 4.1z"/>
                                        </svg>
                                        Refine with AI
                                    </button>
                                    <button className="flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-sm py-3 px-5 rounded-full border border-slate-200 transition-colors w-full">
                                        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                        ...
                        <WeatherDetails PlaceName={trips.PlaceName} />

                        <div className="my-8 mx-auto w-full max-w-7xl px-2">
                            <h3 className="text-xl font-bold text-slate-900 mb-4">Route Map (Day 1 Timeline)</h3>
                            <div className="w-full h-[400px] md:h-[500px] rounded-3xl overflow-hidden border border-slate-200 shadow-sm relative">
                                {ScriptLoaded ? (
                                    <div ref={MapRef} className="w-full h-full" />
                                ) : (
                                    <div className="w-full h-full bg-slate-100 animate-pulse flex items-center justify-center text-slate-400 font-medium">
